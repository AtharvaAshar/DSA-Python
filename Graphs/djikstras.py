...
class Graph:
    ...
    def djikstras(self):
        ...
        distance = [sys.maxsize for _ in range(self.n_vertices)]
        distance[0] = 0
        for i in range(self.n_vertices-1):
            ...
            for j in range(self.n_vertices):
                if self.adjMat[min_vertex][j] > 0 and visited[j] is False:
                    if distance[j] > distance[min_vertex]+self.adjMat[min_vertex][j]:
                        distance[j] = distance[min_vertex] + self.adjMat[min_vertex][j]
